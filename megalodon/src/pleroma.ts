...
import type WebSocket from './pleroma/web_socket.js'
import { type MegalodonInterface, NotImplementedError, ArgumentError } from './megalodon.js'
...
export default class Pleroma implements MegalodonInterface {
	 ...
	 */
	public async registerApp(client_name: string, options: Partial<{ scopes: Array<string>; redirect_uris: string; website: string }>): Promise<OAuth.AppData> {
	 ...
	 */
	public async createApp(client_name: string, options: Partial<{ scopes: Array<string>; redirect_uris: string; website: string }>): Promise<OAuth.AppData> {
		const scopes = options.scopes || DEFAULT_SCOPE
		...
		return this.client.post<PleromaOAuth.AppDataFromServer>('/api/v1/apps', params).then((res: Response<PleromaOAuth.AppDataFromServer>) => PleromaOAuth.toAppData(res.data))
	}
	 ...
	 */
	public generateAuthUrl(clientId: string, clientSecret: string, options: Partial<{ scope: Array<string>; redirect_uri: string | null }>): Promise<string> {
	 ...
	 */
	public async fetchAccessToken(client_id: string | null, client_secret: string, code: string, redirect_uri: string = NO_REDIRECT): Promise<OAuth.TokenData> {
		if (!client_id) {
	 ...
	 */
	public async registerAccount(username: string, email: string, password: string, agreement: boolean, locale: string, reason?: string | null): Promise<Response<Entity.Token>> {
		let params = {
			username: username,
			email: email,
			password: password,
   ...
   */
	 ...
	 * @return Array of statuses.
	 */
	public async getBookmarks(options?: { limit?: number; max_id?: string; since_id?: string; min_id?: string }): Promise<Response<Array<Entity.Status>>> {
		let params = {}
		if (options) {
			if (options.max_id) {
				params = Object.assign(params, {
   ...
   */
	 ...
	 * @return Attachment
	 */
	public async uploadMedia(file: any, options?: { description?: string; focus?: string }): Promise<Response<Entity.Attachment | Entity.AsyncAttachment>> {
	 ...
	 */
	public async getScheduledStatuses(options?: { limit?: number | null; max_id?: string | null; since_id?: string | null; min_id?: string | null }): Promise<Response<Array<Entity.ScheduledStatus>>> {
		let params = {}
		if (options) {
			if (options.limit) {
				params = Object.assign(params, {
	 ...
	 * @return Marker.
	 */
	public async saveMarkers(options?: { home?: { last_read_id: string }; notifications?: { last_read_id: string } }): Promise<Response<Entity.Marker>> {
		let params = {}
		if (options) {
			if (options.home) {
				params = Object.assign(params, {
	 ...
	 * @return Array of accounts.
	 */
	public async getInstanceDirectory(options?: { limit?: number; offset?: number; order?: 'active' | 'new'; local?: boolean }): Promise<Response<Array<Entity.Account>>> {
		let params = {}
		if (options) {
			if (options.limit) {
				params = Object.assign(params, {
