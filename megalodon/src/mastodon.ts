...
export default class Mastodon implements MegalodonInterface {
  ...
  public async getIntegratedTimeline(options?: {
    ...
    const home = await this.client.get<Array<MastodonAPI.Entity.Status>>('/api/v1/timelines/home', params)
    const local = await this.client.get<Array<MastodonAPI.Entity.Status>>(
      '/api/v1/timelines/public',
      Object.assign(params, { local: true })
    )
    const merged = home.data.concat(local.data)
    merged.sort((a, b) => {
      return dayjs(b.created_at).diff(dayjs(a.created_at))
    })
    const sliced = merged.slice(0, options && options.limit ? options.limit : 20).map(s => MastodonAPI.Converter.status(s))
